...
export const retrieveAndTransformData = async (
    periodCodes: string[],
...
export const transformData = (
    ...
    if (
        xIds.length === 1 &&
        xIds.includes(CHART_AXIST_ORGUNITS._id) &&
        yIds.length === 1 &&
        yIds.includes(CHART_AXIST_PERIODS._id)
    ) {
    ...
    if (
        ...
        xIds.includes(CHART_AXIST_ORGUNITS._id) &&
        yIds.length === 1 &&
        yIds.includes(CHART_AXIST_DATA_ELEMENTS._id)
    ) {
    ...
    if (
        ...
        return transformData_OuX_DeAndPeY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
    ...
    if (
        xIds.length === 1 &&
        xIds.includes(CHART_AXIST_PERIODS._id) &&
        yIds.length === 1 &&
        yIds.includes(CHART_AXIST_DATA_ELEMENTS._id)
    ) {
    ...
    if (
        ...
        xIds.includes(CHART_AXIST_PERIODS._id) &&
        yIds.length === 1 &&
        yIds.includes(CHART_AXIST_ORGUNITS._id)
    ) {
    ...
    if (
        ...
        return transformData_PeX_OuAndDeY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
    ...
    if (
        xIds.length === 1 &&
        xIds.includes(CHART_AXIST_DATA_ELEMENTS._id) &&
        yIds.length === 1 &&
        yIds.includes(CHART_AXIST_ORGUNITS._id)
    ) {
    ...
    if (
        ...
        return transformData_DeX_OuAndPeY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
    ...
    if (
        ...
        return transformData_OuAndDeX_PeY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
    ...
    if (
        ...
        return transformData_OuAndPeX_DeY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
    ...
    if (
        ...
        return transformData_DeAndPeX_OuY(
            data,
            sortedOrgUnits,
            sortedDataElements,
            sortedPeriods,
...
const transformData_OuX_PeY = (
    ...
    for (var i = 0; i < orgUnits.length; i++) {
        ...
        for (var j = 0; j < periods.length; j++) {
            ...
            const dataValuesByDE = getDataValuesByPeriod(
                dataValuesByOU,
                period,
            );
            const total = dataValuesByDE.reduce(
                (sum, dataValue) => sum + (dataValue.value || 0),
                0,
            );
...
const transformData_OuX_DeY = (
    ...
    for (var i = 0; i < orgUnits.length; i++) {
        ...
        for (var j = 0; j < dataElements.length; j++) {
            ...
            const dataValuesByDE = getDataValuesByDataElement(
                dataValuesByOU,
                dataElement,
            );
 ...
 * ]
 */
const transformData_OuX_DeAndPeY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < orgUnits.length; i++) {
        ...
        for (var j = 0; j < axisYList.length; j++) {
            ...
            const dataValuesByDE = getDataValuesByDE_Period(
                dataValuesByOU,
                axisYItem['period'],
                axisYItem['dataElement'],
            );
            const total = sumValues(dataValuesByDE);
    ...
    return {
        chartData: result,
        axisY: axisYList.map(
            (item) => `${item['period'].name}-${item['dataElement'].name}`,
        ),
    };
};
...
const transformData_PeX_OuY = (
    ...
    for (var i = 0; i < periods.length; i++) {
        ...
        for (var j = 0; j < orgUnits.length; j++) {
            ...
            const dataValuesByOU = getDataValuesByOrgUnit(
                dataValuesByPeriod,
                orgUnitData,
            );
            const total = dataValuesByOU.reduce(
                (sum, dataValue) => sum + (dataValue.value || 0),
                0,
            );
 ...
 * ]
 */
const transformData_PeX_OuAndDeY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < periods.length; i++) {
        ...
        for (var j = 0; j < axisYList.length; j++) {
            const axisYItem = axisYList[j];
            const dataValuesByOU_PE = getDataValuesByOU_DE(
                dataValuesByPeriod,
                axisYItem['orgUnit'],
                axisYItem['dataElement'],
            );
    ...
    return {
        chartData: result,
        axisY: axisYList.map(
            (item) => `${item['orgUnit'].name}-${item['dataElement'].name}`,
        ),
    };
};
...
const transformData_DeX_PeY = (
    ...
    for (var i = 0; i < dataElements.length; i++) {
        ...
        for (var j = 0; j < periods.length; j++) {
            const period = periods[j];
            const dataValuesByOU = getDataValuesByPeriod(
                dataValuesByDE,
                period,
            );
            const total = dataValuesByOU.reduce(
 ...
 * ]
 */
const transformData_DeX_OuAndPeY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < dataElements.length; i++) {
        ...
        const dataValuesByPeriod = getDataValuesByDataElement(
            data,
            dataElement,
        );
        for (var j = 0; j < axisYList.length; j++) {
            const axisYItem = axisYList[j];
            const dataValuesByOU_PE = getDataValuesByOU_Period(
                dataValuesByPeriod,
                axisYItem['orgUnit'],
                axisYItem['period'],
            );
    ...
    return {
        chartData: result,
        axisY: axisYList.map(
            (item) => `${item['orgUnit'].name}-${item['period'].name}`,
        ),
    };
};
 ...
 * ]
 */
const transformData_OuAndDeX_PeY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < axisXList.length; i++) {
        ...
        const item: JSONObject = {
            axisX: `${axisXItem['orgUnit'].name} - ${axisXItem['dataElement'].name}`,
        };

        const dataValuesByPeriod = getDataValuesByOU_DE(
            data,
            axisXItem['orgUnit'],
            axisXItem['dataElement'],
        );
        for (var j = 0; j < periods.length; j++) {
            const period = periods[j];
            const dataValuesByPE = getDataValuesByPeriod(
                dataValuesByPeriod,
                period,
            );
 ...
 * ]
 */
const transformData_OuAndPeX_DeY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < axisXList.length; i++) {
        ...
        const item: JSONObject = {
            axisX: `${axisXItem['orgUnit'].name} - ${axisXItem['period'].name}`,
        };
 ...
 * ]
 */
const transformData_DeAndPeX_OuY = (
    data: JSONObject[],
    orgUnits: JSONObject[],
    dataElements: IDataElement[],
    periods: ISerializePeriod[],
    ...
    for (var i = 0; i < axisXList.length; i++) {
        ...
        const item: JSONObject = {
            axisX: `${axisXItem['dataElement'].name} - ${axisXItem['period'].name}`,
        };

        const dataValuesByOU_PE = getDataValuesByDE_Period(
            data,
            axisXItem['dataElement'],
            axisXItem['period'],
        );
...
const getDataValuesByOrgUnit = (
    data: JSONObject[],
    orgUnitData: JSONObject,
) => {
    return data.filter(
        (dataValue: JSONObject) => dataValue.orgUnit === orgUnitData._id,
    );
};

const getDataValuesByDataElement = (
    data: JSONObject[],
    dataElement: IDataElement,
) => {
    return data.filter(
        (dataValue: JSONObject) =>
            dataValue.dataElement._id === dataElement._id,
    );
};

const getDataValuesByPeriod = (
    data: JSONObject[],
    period: ISerializePeriod,
) => {
    return data.filter(
        (dataValue: JSONObject) => dataValue.period.code === period.code,
    );
};
...
const getDataValuesByOU_Period = (
    data: JSONObject[],
    orgUnit: JSONObject,
    period: ISerializePeriod,
) => {
    return data.filter(
        (dataValue: JSONObject) =>
            dataValue.orgUnit === orgUnit._id &&
            dataValue.period.code === period.code,
    );
};

const getDataValuesByDE_Period = (
    data: JSONObject[],
    period: ISerializePeriod,
    dataElement: IDataElement,
) => {
