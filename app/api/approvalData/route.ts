    ...
    try {
        ...
        if (!dataSet || !periodCode || !orgUnit) {
            return Response.json(
                {
                    message:
                        'Missing required fields: dataSet, period, and orgUnit. Please ensure all required fields are included in your request.',
                },
                { status: 500 },
            );
        }
        ...
        let result = await ApprovalData.findOne(condition).populate(
