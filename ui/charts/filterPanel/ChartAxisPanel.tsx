...
const categories = [CHART_AXIST_ORGUNITS, CHART_AXIST_DATA_ELEMENTS, CHART_AXIST_PERIODS];
...
export default function ChartAxisPanel({
    ...
    const { selectedChartX, selectedChartY, selectChartX, selectChartY } = useChart();
...
const MutipleSelector = (
{
    ...
    onChange: (e: React.ChangeEvent<HTMLInputElement>, item: IChartAxist) => void
}) => {
    return (
        <div className="flex flex-col space-y-2">
            ...
            {selected.length == 0 && <span className="text-red-600 italic text-sm">This field is required</span>}
        </div>
    )
}