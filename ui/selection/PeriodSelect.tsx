...
export default function PeriodSelect( {periodType}: {periodType: string} ) {
    ...
    return (
         <div className="relative w-64 h-full">
            ...
            {showed && <div className="absolute z-50 top-10 left-0 right-0 bg-white shadow-lg h-96">
                ...
                <div className="border border-slate-200 min-h-96 overflow-y-auto">
                    <ul className="border border-slate-200 max-h-80">
                        {periods.map((item: IPeriod) => (
                            <li key={item.name} 
                                className={`cursor-pointer py-5 px-3 border-y border-slate-200 bg-white hover:bg-blue-100 ${selectedPeriod && selectedPeriod.name === item.name && "bg-slate-200"}`}
